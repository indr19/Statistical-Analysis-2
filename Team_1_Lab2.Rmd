...
```{r}
suppressWarnings(setwd("~/Desktop/Stone/Berkeley_MIDS/Statistics/github/lab_2/"))
...
We wanted to answer this question from the perspective of each individual voter, as each voter was asked how they would rate the police versus journalists (a natural pairing). Namely, for each voter, do they respect the police or journalists more? To address this, we require that all respondents answer both questions, and filtered out those that did not answer either or both question (response -7). Also, to narrow the sample down to US voters, we only looked at individuals who were registered to vote. These are many reasons why someone might choose to not vote in an election, including lack of interest in the voter decision, lack of strong support for either opposition, or simply forgetting. However, if one is eligible to vote, they should be classified as US voters, since these individuals have the ability and opportunity to cast their decision in upcoming elections. There were only two individuals who did not respond to ftjournal, and all responded to ftpolice. In addition, 316 were not registered to vote. This reduces our dataset to 1746 participants, still a sizable number.
...
Since the p-value is >0.05, we fail to reject the null hypothesis that the population of ages come from the same population. The p-value is very close to 0.05. As a result, when performing our two sample t-test, we will use treat the variances for the Republican voter age population and Democratic voter age population as the same.
...
We get an effect size close 0.064, which is very small. Typically, the ranges are <0.15 is small, 0.15-0.25 is medium, and >=0.25 is large. We can conclude that our study is highly statistically significant with low level of practical significance. This makes sense since almost 46% of the sample believe the investigation was baseless. This is very close to 50%, but certainly still not a majority.
...
Our proposed question is:

For voters that did not vote in the 2016 Presidential Election, but anticipate themselves to vote in the 2020 Presidential Election, did a larger proportion of these individuals change their behavior because they have been satisfied with Trump's performance as president (and therefore would likely vote for these individuals), or vice-versa? 

We believe this is a fundamental question in voting behavior, which is whether voters more likely to show up to the polls to support a candidate they strongly favor, or are they more likely to show up to the polls to vote against candidates they strongly dislike? The stakes are likely viewed by many as the highest for the Presidential election, and we have precisely the data to address this question. Understanding this question will likely help campaigns create more targeted ads. For example, if we find that individuals are more likely to show up to support someone they favor, then campaign ads should focus more on promoting individuals they support rather than attacking the opposing party. This question will also help advocacy groups devoted to increasing voter turnout. For example, if we find that individuals are more likely to show up to vote against someone they dislike, advocacy groups might focus on iterating how not voting might result in a candidate entering the office that the potential voter strongly dislikes.

To address this question, we will look at individuals who were of voting age in 2016, and select the proportion of these individuals who did not vote in 2016 but anticipate to vote in 2020. The first variable is turnout16, and the second is percent20. Unfortunately, percent20 is a percentage from 1 to 100, so we will classify individuals who are >50 to be ones who will likely vote. >50 is the best decision boundary between which we anticipate whether or not the individual will vote. To evaluate whether or not the voter preferred Trump, we will look at the apppres variable. The variable is categorical and ranges from 1 to 7, from Approve extremely strongly to Disapprove extremely strongly. All individuals that selected "Approve" to some level (1-3) will be classified as favoring Trump, and all individuals that selected "Disapprove" to some level (5-7) will be classified as disfavoring Trump.

We filter our set according to the specifications above:
```{r}
non_voters_2020 <- A %>% filter(turnout16==2, percent20>50, birthyr<=1998) %>%
                         mutate(party = ifelse(pid7x == 1 | pid7x == 2, 'D',
                                ifelse(pid7x == 6 | pid7x == 7, 'R', 
                                ifelse(pid7x == 3 | pid7x == 4 | pid7x == 5, 'I', 'N')))) %>%
                         mutate(age = 2018 - birthyr) %>%
                         mutate(approve_trump = ifelse(apppres >= 5, 'N', 
                                                ifelse(apppres <=3, 'Y', 'Not'))) %>%
                         filter(approve_trump != 'Not') %>%                      
                         select(age, party, approve_trump)
```

We first want to see whether or not our sample will be biased in parties in terms of Democrat versus Republican:
...
We see that we have very similar amounts of individuals in either Democratic or Republican parties. Interestingly, more than half of the sample are Independent voters, and only 1 whose party was not disclosed. This is an excellent result because it means that we are not biasing our sample to either Trump's party, or the party that is the biggest opponent of Trump.

We can plot a bar plot to visualize how many of each group we have:
...
We see that almost double of these individuals disapprove of Trump to some degree.

Finally, let's check the integrity of the data before proceeding to the hypothesis test:

```{r}
summary(non_voters_2020)
```

Our age varible has a minimum of 20, meaning this individual was of voting age in 2016. All other variables are as expected.

The most appropriate test here is the binomial test for proportion. The assumptions of this test are:

1. Samples are dichotomous and nominal. This is certainly the case here after the transformation: the subjects are classified as either approve or disapprove of Trump, which does not have a numerical value associated with them.

2. iid samples. This is the case since each subject's decision in the survey does not influence other subjects. In terms of identical distribution, this can be assumed since all subjects are drawn from the population of survey takers.

More explicitly, let R represent the proportion of individuals that did not vote in 2016 but most likely will vote in 2020 that approve Trump. We will test at a significance level of alpha = 0.05 to see if L was 0.5. We also note that we will use a two-tailed test since we do not have prior evidence that fear or anger is a more important factor for this group of individuals. 
...
We see that the p-value is very small, much smaller than 0.05. This means that we reject H0 as we have a very low chance under H0 to see data as extreme as our sample. The conclusion is that the proportion of individuals that did not vote in 2016 but most likely will vote in 2020 who favored Trump is not 0.5. We can see from the bar plot that more individuals disapprove of Trump, so we can conclude one step further than more individuals statistically disapprove of Trump than those that approve of Trump.
...
We get an effect size slightly greater than 0.15, which is medium effect size. We can conclude that our study is highly statistically significant with medium level of practical significance. This makes sense since almost more than double the number of individuals disapprove of Trump than approve of Trump, equating to about 65.6% of the sample. We can say that this is appears to be a majority.