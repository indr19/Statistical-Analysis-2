...
The most appropriate test here is the Wilcoxon signed-rank test. Modern research shows that this particular test works perfectly fine for ordinal data. The assumptions of this test are:
...
=======
We can see that the minimum value is -7 and maximum is 7, meaning some subset of Independent voters skipped this questions. However, there is only a single respondent who skipped this question, so we need to remove that individual. There are a total of 429 individuals who believed that Russia probably interfered (there was a base), and 360 that believed that Russia probably did not (baseless). We can generate a bar plot of this binary variable.
>>>>>>> origin/master
...
=======
We get an effect size close 0.042, which is very small. Typically, the ranges are <0.15 is small, 0.15-0.25 is medium, and >=0.25 is large. We can conclude that our study is highly statistically significant with low level of practical significance. This makes sense since almost 46% of the population believe the investigation was baseless. This is very close to 50%, but certainly still not a majority.
>>>>>>> origin/master
...
```{r}
summary(AG_FR_turnout$geangry)
summary(AG_FR_turnout$geafraid)
```

We see that both variables have the same logical min/max (based on possible values from the survey). The median for both is 3, but the mean for afraid and 3rd quartile for afraid were a little bit higher. Since we are measuring two ratings from the same individual, we can now generate a new variable that measures the difference between anger and fear levels based on this ordinal scale, and look at its summary:

```{r}
AG_FR_turnout$angry_minus_fear <- AG_FR_turnout$geangry - AG_FR_turnout$geafraid
print(summary(AG_FR_turnout$angry_minus_fear))
```

The median is centered around 0, and mean is slightly negative, favoring anger over fear. We also do a sanity check that all ages are greater than or equal to 20 in the year 2018:
...
2. The differences are measured on at least an ordinal scale. Note that since this is the case. Our variables are naturally binned into an ordinal scale. As a result, we can subtract the ranks and look at, for each respondent, what the difference between anger and fear levels for 2018 voters that did not vote in 2016. 

3. The distribution is symmetric around its mean and median. This is true under our null hypothesis, which is that the difference between the ranks of the two groups is 0. Possible values that the difference takes from integers going from -5 to +5. 

More explicitly, let L represent the r.v. of the difference between anger and fear levels for our group of interest. We will test at a significance level of alpha = 0.05, as it is a commonly used value. We also note that we will use a two-tailed test since we do not have prior evidence that fear or anger is a more important factor for this group of individuals.
...
To conduct the test, we use the following:
```{r}
wilcox.test(AG_FR_turnout$geangry, 
            AG_FR_turnout$geafraid,     
            paired = TRUE)
```

We see that there's a very large p-value much greater than alpha. This means that we fail to reject H0 since we had a very high chance under H0 to see data as extreme as our sample. The conclusion is that neither anger nor fear levels were greater than the other within this group of individuals, and so we can propose that neither fear nor anger was more effective at driving voter turnout.
...
Our proportion approach produces an effect size
```{r }
p.value <- 0.9601348 #from wilcox.test
z.score <- -qnorm(p.value/2)
effect.correlation <- z.score / sqrt(length(AG_FR_turnout$angry_minus_fear))
print(effect.correlation)
```

The effect correlation r is very small, essentially negligible, as it is less than 0.1. As a result, there is neither a statistical or practical difference between anger and fear driving voter turnout.
